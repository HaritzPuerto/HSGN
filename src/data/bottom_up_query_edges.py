...
#             assert span_idx == (-1, -1)
    ...
    def create_graph(self, hotpot_instance, list_entities, dict_idx, context, ins_idx):
        ...
        list_ent_multihop = []
        list_q_srl2srl = []
        list_query2sent_pred = []  # connect the query to all sentences that contains the same predicates
        ...
        list_query_preds = self.get_query_predicates(ins_idx)
        # for each doc
...
#                                                       for arg2 in list_arg_nodes])
                    ...
                    # query -> srl # lbl:[QUERY2SRL_shared_pred]
                    if verb in list_query_preds:
                        list_query2srl_shared_pred.extend([(0, current_sent_node) for arg in list_arg_nodes])  
...
#         list_doc2doc.extend([(v, u) for u in list_doc_nodes for v in list_doc_nodes if u != v])
        ...
        if list_query2srl_shared_pred != []:
            dict_edges['query', 'query2srl_pred', 'srl'] = list_query2srl_shared_pred
...
#         graph.nodes['tok'].data['labels'] = np.array(list_token_lbl).reshape(-1,1)
    ...
    def get_query_predicates(self, ins_idx):
        list_preds = []
        for key, triple_dict in self.dict_ins_query_triples[str(ins_idx)].items():
            if 'V' in triple_dict:
                list_preds.append(triple_dict['V'])
        return list_preds
...
#         print(list_ent_multihop)
    ...
    def get_sent_predicates(self, ins_idx, doc_idx, sent_idx):
        set_preds = set()
        dict_srl = self.dict_ins_doc_sent_srl_triples[str(ins_idx)][str(doc_idx)][str(sent_idx)]
        for triple in dict_srl.values():
            for arg_type, arg in triple.items():
                if arg_type == 'V':
                    set_preds.add(arg)
        return set_preds
    ...
    def get_query_predicates(self, ins_idx):
        set_preds = set()
        for triple in self.dict_ins_query_triples[str(ins_idx)].values():
            for arg_type, arg in triple.items():
                if arg_type == 'V':
                    set_preds.add(arg.lower())
        return set_preds

    def get_edges_common_preds(self, ins_idx, doc_idx, sent_idx):
        set_preds_query = self.get_query_predicates(ins_idx)
        set_preds_sent = self.get_sent_predicates(ins_idx, doc_idx, sent_idx)
        common_preds = set_preds_sent.intersection(set_preds_query)
...
# %%
training_path = os.path.join(data_path, 'processed/training/heterog_20201103_q_srl_shared_pred')
...
# # %%
# dev_path = os.path.join(data_path, 'processed/dev/heterog_20201103_q_srl_shared_pred')
