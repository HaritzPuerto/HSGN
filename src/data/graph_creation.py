...
class Dataset():
    def __init__(self, dataset=None, list_hotpot_ner=None, dict_ins_doc_sent_srl_triples=None,
        ...
        self.dict_ins2dict_doc2pred = dict_ins2dict_doc2pred
    ...
    def create_dataloader(self):
        ...
        for ins_idx, hotpot_instance in enumerate(tqdm(self.dataset)):
            ...
            if 'srl2srl' in g.etypes:
                g.edges['srl2srl'].data['rel_type'] = torch.tensor([edge['rel_type'] for edge in list_srl_edges_metadata])
                g.edges['srl2srl'].data['span_idx'] = torch.tensor([edge['span_idx'] for edge in list_srl_edges_metadata])
    ...
    def batch_encoding(self, dataset):
        ...
        for hotpot_idx, hotpot_instance in enumerate(self.dataset):
            ...
            for doc_idx, (doc_title, doc) in enumerate(hotpot_instance['context']):
                if self.dict_ins2dict_doc2pred[hotpot_idx][doc_idx] != 1:
                    continue
...
#                 print("### sent", lbl, sent_str)
                ########################################################################
                # for each SRL triple
                for key, triple_dict in self.dict_ins_doc_sent_srl_triples[ins_idx][doc_idx][sent_idx].items():
...
#         list_doc2doc.extend([(v, u) for u in list_doc_nodes for v in list_doc_nodes if u != v])
        ...
        for key, triple_dict in self.dict_ins_query_triples[ins_idx].items():
...
#         print(list_ent_multihop)
    ...
    def get_sent_predicates(self, ins_idx, doc_idx, sent_idx):
        set_preds = set()
        dict_srl = self.dict_ins_doc_sent_srl_triples[ins_idx][doc_idx][sent_idx]
    ...
    def get_query_predicates(self, ins_idx):
        set_preds = set()
        for triple in self.dict_ins_query_triples[ins_idx].values():
            for arg_type, arg in triple.items():
                if arg_type == 'V':
