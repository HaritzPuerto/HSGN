...
class Dataset():
    def __init__(self, dataset = None, list_hotpot_ner = None, dict_ins_doc_sent_srl_triples = None,
                 dict_ins_query_triples = None, list_entities_query = None, top4_rel_docs=None, batch_size = None, max_len = 512):
        ...
        self.top4_rel_docs = top4_rel_docs
...
#             assert span_idx == (-1, -1)
    ...
    def batch_encoding(self, dataset):
        ...
        for hotpot_idx, hotpot_instance in enumerate(self.dataset):
            ...
            set_supporting_doc_idx = topk_preds(hotpot_idx, self.dataset, self.top4_rel_docs)
            # for each doc
            yes_no_added = False
            for doc_idx, (doc_title, doc) in enumerate(hotpot_instance['context']):
                if doc_idx not in set_supporting_doc_idx:
                    continue
